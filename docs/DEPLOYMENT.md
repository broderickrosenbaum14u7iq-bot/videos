...
**This VPS now hosts more than one independent site** on this same codebase (`dongtoico.org` alongside `phimtoico.org`, sharing the VPS, MySQL server, and Redis server, each with its own database and Redis logical database index). This document covers deploying a *release* to an *already-provisioned* site; for cloning this codebase onto a brand-new domain from scratch, use `docs/DEPLOY_NEW_SITE.md` instead — it covers exactly the per-site isolation (database, Redis index, cron path, R2/Stream config) that a second site actually needs, learned from real setup bugs the first time this happened.

**Production deployment by an AI assistant is prohibited by default. It is permitted only when the project owner explicitly authorizes a production deployment in the current conversation. Before modifying production, the assistant must create or verify a rollback point/backup, preserve persistent production data and secrets, deploy only an explicitly approved commit/tag, perform a production health check, and rollback on a severe regression.** Absent that explicit, in-conversation authorization, this document is for the human operator (or a reviewed, human-triggered deploy script) to execute. Per `DEVELOPMENT_RULES.md` §3, all implementation and verification happens in the local Docker staging environment; production is only ever touched through this documented, reviewed procedure.
...
- [ ] **Clean-checkout boot test**: in a scratch directory (not the long-lived staging checkout, which accumulates `vendor/` directories built up over the project's history and can mask a packaging gap), `git clone` the tagged commit fresh, run the §3 step 2 per-plugin `composer install` loop, point a throwaway WordPress install at it, and confirm **every plugin under `wp-content/plugins/`** activates with zero fatals — verify the count matches `ls wp-content/plugins/*/composer.json | wc -l` (nine at the time of writing: `tube-core`, `tube-cache`, `tube-player`, `tube-search`, `tube-seo`, `tube-admin`, `tube-members`, `tube-ads`, `tube-comments`) rather than trusting a remembered number, since a plugin count baked into this checklist is exactly what silently drifted before (2026-08-28 P0 remediation: the §3 loop below had hardcoded six plugin names and fatal-errored the three added since). A staging environment that has never had its `vendor/` directories deleted is not a substitute for this — it will boot successfully even if the release's actual deploy procedure is broken, which is exactly how the v1.0.0 tag shipped without this step documented.
...
2. **Run `composer install --no-dev --optimize-autoloader` separately inside every plugin directory that has its own `composer.json`** — this project has no shared runtime autoloader (`ARCHITECTURE.md` §4: each plugin must remain independently `composer install`-able; the repo-root `composer.json` is dev tooling only — PHPCS/PHPStan — and has no `autoload` section at all, so running `composer install` only at the release root silently leaves every plugin's `vendor/autoload.php` missing and every plugin fatals on boot with `Class "Tube_X\Plugin" not found`. This is not optional per plugin — even the plugins with zero third-party packages (`tube-player`, `tube-search`, `tube-seo`, `tube-admin`) still need their own generated `vendor/autoload.php`, since that file is what registers their `Tube_X\` PSR-4 namespace; nothing else in the boot path does):
   ```sh
   ...
   ```
   **Discover the plugin list from `wp-content/plugins/*/composer.json` at deploy time — never hardcode the plugin names in this loop.** A hardcoded list (`tube-core tube-cache tube-player tube-search tube-seo tube-admin`) is exactly what caused a P0 release blocker (2026-08-28 audit): three plugins added later (`tube-members`, `tube-ads`, `tube-comments`) were silently skipped by every deploy that copy-pasted this loop, fatal-erroring login, comments, and ad serving simultaneously on first production boot. The glob above installs every plugin that has its own `composer.json`, present today or added tomorrow, with no step here to remember to update.
   `--no-dev` excludes every plugin's `phpunit`/dev-only dependency (every plugin's `composer.json` keeps `phpunit/phpunit` under `require-dev` only). Every plugin's `composer.lock` is git-tracked, so this is a reproducible `install`, never an `update` — the exact locked versions reviewed in staging are what ships to production.
...
- [ ] A real video page loads: `curl -o /dev/null -w '%{http_code}' https://<domain>/watch/<a-real-slug>/` → `200`, and its rendered HTML includes a `tube-comments`-owned selector (e.g. `class="tube-comments"`) — confirms the comments plugin actually loaded and rendered, not just that the page returned 200.
...
- [ ] `POST /wp-json/tube/v1/auth/login` with a deliberately-wrong password returns `401`/`422` (a real JSON error response, not a PHP fatal or a raw WSOD) — confirms `tube-members` loaded.
- [ ] Any page with an active ad placement renders without a PHP fatal in the response body — confirms `tube-ads` loaded. (Skip if no placement is configured yet on this deploy.)
- [ ] `wp tube migrate status --path=<new-release-path>` shows every migration `applied`.
- [ ] `wp --path=<new-release-path> plugin list` shows every plugin `active` at the deployed version, and the row count matches `ls wp-content/plugins/*/composer.json | wc -l` for that same release directory — not a remembered number (see §1's clean-checkout boot test for why).
