...
2. Deploy sequence: pull the tagged release → `composer install --no-dev` → confirm CI's PHPCS gate already passed → `wp tube migrate status` to preview pending schema changes → `wp tube migrate up` → deploy code via an **atomic symlink swap** (release directory + a symlink flip to the current release) so a bad deploy can be reverted instantly by re-pointing the symlink, without a redeploy → smoke test (homepage loads, a video page loads, `GET /tube/v1/videos` responds) → purge only what actually changed (versioned asset filenames make most cache purging unnecessary) → watch error logs for the following 15 minutes.
