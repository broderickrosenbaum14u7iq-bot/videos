 ...
 *
 * Deliberately does not use the main WP_Query loop — the video listing
 * comes from tube-search's index (`tube_search_by_category()`) instead,
 * per this phase's "avoid unnecessary WP_Query" instruction. Only the
 * term's own name/description (a cheap, already-cached term lookup) uses
 * native WordPress taxonomy functions.
 *
 * @package Tube_Theme
 */
...
if ($tube_theme_term instanceof WP_Term) {
    ...
    $tube_theme_result = tube_search_by_category($tube_theme_term->term_id, $tube_theme_page);
    ...
    get_template_part(
        ...
        [
            ...
            'page_url'      => static fn (int $tube_theme_target_page): string => $tube_theme_target_page > 1
                ...
                : $tube_theme_base_url,
            'empty_message' => __('Chưa có video nào trong danh mục này.', 'tube-theme'),
        ]
    );
}
