 ...
 * install (tube-core and tube-admin active, per `Requires Plugins`) so
 * tests can exercise real $wpdb/admin-hook-coupled logic. Must run
 * inside the wpcli Docker container — mirrors every other plugin's own
...
(static function (): void {
    $wp_load_path = getenv('TUBE_ADMIN_WP_LOAD_PATH');
    ...
    if (! is_readable($wp_load_path)) {
        ...
        fwrite(
            ...
            "(set TUBE_ADMIN_WP_LOAD_PATH to override the expected wp-load.php path)\n"
        );
        exit(1);
    }
