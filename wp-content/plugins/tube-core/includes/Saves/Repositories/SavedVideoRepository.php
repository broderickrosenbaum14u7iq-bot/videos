<?php
/**
 * Data access for wp_tube_saved_videos (SavedVideoRepositoryInterface) implementation.
 *
 * @package Tube_Core
 */
...
/**
 * Data access for `wp_tube_saved_videos` (SavedVideoRepositoryInterface)
 * implementation — identical shape to `LikeRepository`, see its
 * docblock for why every method branches into a user-identity or a
 * visitor-token-identity query rather than one query with an `OR`.
 */
final class SavedVideoRepository implements SavedVideoRepositoryInterface
{
    /**
     * {@inheritDoc}
     *
    ...
    public function has_saved(?int $user_id, ?string $visitor_token, int $video_id): bool
    {
        ...
        $table = $wpdb->prefix . 'tube_saved_videos';
    ...
    public function video_ids_for_user(int $user_id, int $limit): array
    {
        ...
        $video_ids = $wpdb->get_col(
            $wpdb->prepare(
                'SELECT video_id FROM %i WHERE user_id = %d ORDER BY id DESC LIMIT %d',
                $table,
                $user_id,
                $limit
            )
        ...
        return array_values(array_map(static fn ($id): int => is_numeric($id) ? (int) $id : 0, $video_ids));
    }
}
