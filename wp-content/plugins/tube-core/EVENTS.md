...
| `VIDEO_STREAM_STATUS_CHANGED` | `tube_core.video.stream_status_changed` | **Active** (Phase 5) | A Cloudflare Stream webhook reports a status genuinely different from what's stored — a redelivered webhook reporting the same status does not re-fire this. | `['video_id' => int, 'status' => string]` | tube-admin (dashboard update) |
| `VIDEO_VIEW_RECORDED` | `tube_core.video.view_recorded` | **Active** (Phase 4) | `ViewRecorder::record()` buffers a view. Internal only per §6 — the stats rollup job reads `wp_tube_video_views` directly rather than subscribing. | `['video_id' => int]` | (none — internal) |
| `VIDEO_STATS_ROLLED_UP` | `tube_core.video.stats_rolled_up` | **Active** (Phase 4) | The end of `wp tube-core stats:rollup`, once per video with a statistics row (including videos with zero recent views). | `['video_id' => int, 'views_total' => int]` | tube-search (refresh `views_total` in the index) |
| `IMPORT_ITEM_COMPLETED` | `tube_core.import.item_completed` | **Active** (Phase 5) | An `import_queue` item finishes successfully. | `['queue_id' => int, 'video_id' => int]` | tube-admin (dashboard), tube-search (index insert) |
| `IMPORT_ITEM_FAILED` | `tube_core.import.item_failed` | **Active** (Phase 5) | An `import_queue` item exhausts its retry attempts — not fired on an attempt that will still be retried. | `['queue_id' => int, 'error_message' => string]` | tube-admin (alert/dashboard) |
