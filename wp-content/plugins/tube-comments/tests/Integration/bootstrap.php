 ...
 * install (not a WP_UnitTestCase sandbox) so tests can exercise real
 * $wpdb, real WP_User accounts, and the new anti-spam schema
 * (`wp_tube_comment_root_locks`) against a real MySQL/MariaDB instance.
 * Must run inside the wpcli Docker container, which is the only place a
 * full WordPress + MySQL stack is reachable — see docker-compose.yml's
 * `wpcli` service, and `tube-core/tests/Integration/bootstrap.php` for
 * the identical pattern this file mirrors.
 *
...
(static function (): void {
    $wp_load_path = getenv('TUBE_COMMENTS_WP_LOAD_PATH');
    ...
    if (! is_readable($wp_load_path)) {
        ...
        fwrite(
            ...
            "(set TUBE_COMMENTS_WP_LOAD_PATH to override the expected wp-load.php path)\n"
        );
        exit(1);
    }
